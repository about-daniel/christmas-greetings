...
export default function Snow({...props}){
    ...
    useEffect(() => {
        // === THREE.JS CODE START ===
        let scene,camera,renderer,particles,logo;
        ...
        loader.load(
            ...
            function ( glb ) {
                ...
                // tl.to(logo.scale,{x:6,y:6,z:6,duration:0.75,ease:'back'})
                //     .to(logo.rotation,{x:10,duration:0.75,ease:'back'},'-=0.6');
        ...
        const init = () => {
            ...
            gui.add(logoPosition, 0, 100, 10 ); // min, max, step
            logo.position.set(logoPosition,0,0);
            // Snow Particles
            const points = []
    ...
    useEffect(() => {
        // Update Three.js object position when React state changes
        logo.position.set(logoPosition.x, logoPosition.y, logoPosition.z);
      }, [logoPosition]);
    return (
